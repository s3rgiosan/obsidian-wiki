...
### Source 1: `$CLAUDE_HISTORY_PATH/` (CLI sessions)
...
**Conversation transcript (`.claude/projects/.../<uuid>.jsonl`)** — Identical format to CLI conversation JSONL. Parse the same way as `$CLAUDE_HISTORY_PATH/projects/*/*.jsonl`.

### Key data sources ranked by value (both locations combined):

1. **Memory files** (`$CLAUDE_HISTORY_PATH/projects/*/memory/*.md`) — Pre-distilled, already wiki-friendly. Gold.
2. **Conversation JSONL** (both `$CLAUDE_HISTORY_PATH/projects/*/*.jsonl` and desktop app transcripts) — Full conversation transcripts. Rich but noisy.
...
```bash
# --- Source 1: CLI sessions ($CLAUDE_HISTORY_PATH) ---
# Find all projects
